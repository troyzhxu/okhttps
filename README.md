...
> * 拦截器处理时机在请求前和响应后，预处理器只在请求前，并且先于拦截器执行。关于响应后，OkHttps还提供了全局回调机制（详见6.6章节）
...
　　全局回调与拦截器的异同:
> * 拦截器可以添加多个，全局回调分三种，每种最多添加一个
> * 拦截器处的理时机在请求前和响应后，全局回调只在响应后，并且晚于拦截器
> * 全局回调可以阻断（return false）某个请求的具体回调，而拦截器不能
